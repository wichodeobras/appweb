...
export function f3_Mn(
    Clas,     // "Compacto" | "No compacto" | "Esbelto"
    E,        // módulo de elasticidad
    kc,       // coeficiente kc
    Sx,       // sección resistente Sx (N·mm)
    lambda,   // relación b/t
    Mp,       // momento plástico Mp (N·mm)
    Fy,       // esfuerzo de fluencia
    lambdapf, // λp (límite inelástico)
    lambdarf  // λr (límite elástico)
) {
    let Mn;
    if (Clas === "Compacto") {
        // perfiles compactos llegan a Mp
        Mn = Mp;
    } else if (Clas === "No compacto") {
        // interpolación lineal inelástica
        Mn = Mp - (Mp - 0.7 * Fy * Sx) * ((lambda - lambdapf) / (lambdarf - lambdapf));
    } else if (Clas === "Esbelto") {
        // región completamente elástica
        Mn = (0.9 * E * kc * Sx) / (lambda * lambda);
    } else {
        Mn = 0;
    }
    return Mn;
...
export function MomentoResistente (clas, f2Mn, f3Mn) {
    let MnRes;
    if (clas === "Compacto") {
        MnRes = f2Mn;
    } else if (clas === "No compacto" || clas === "Esbelto") {
        MnRes = Math.min(f2Mn, f3Mn);
    } else {
        MnRes = 0;
    }
    return MnRes;
